...
const GOLDEN_DEFAULT_INSTRUCTIONS = [
  ...
  "- When the matching Atrium surfaces are exposed, use them first for repository inspection: search to discover paths or content, then read exact files with the read tool. Do not use a shell or host built-in file reader for the same job when the matching Atrium primitive is callable.",
  "- Call an exposed Atrium tool directly. If the needed Atrium tool is deferred, use the host tool-search mechanism to surface that exact tool, then call it. Instructions cannot register tools; if its surface is absent or remains unavailable, use a fallback only for the specific unsupported operation.",
  "",
  "Read contract:",
  ...
  "- When the read surface is enabled and callable, prefer atrium-read for exact repository files. If it is deferred, use the host tool-search mechanism to surface atrium-read before using a host reader.",
  ...
  "- When the search surface is enabled and callable, prefer Atrium search primitives for repository path and content discovery. If one is deferred, use the host tool-search mechanism to surface the exact primitive before choosing a host search tool.",
  ...
  "",
  "Search-policy is active. Do not call direct search commands or tools: rg, ripgrep, grep, git grep, xray, find, findstr, or Select-String. These primitives also replace the harness built-in glob and grep tools: map glob to atrium-find-files and grep to atrium-grep or atrium-grep-code. When such a built-in is disabled or unavailable, call the Atrium primitive instead of retrying the built-in. Use the Atrium MCP search primitives instead: atrium-find-files for path or name discovery, atrium-grep for content search, and atrium-grep-code for git-aware code search. Pass query as one pattern or an array of patterns to match any of. These Atrium primitives are deferred MCP tools. If atrium-find-files, atrium-grep, or atrium-grep-code is not in your immediately callable tool list, call the tool search tool first (for example with the pattern find-files|grep) to surface them, then call them. Do not abandon the search or read files blindly because a primitive looks unavailable. Always pass query and an explicit location: root, or an absolute path; narrow with glob, exclude, and max. If a search times out, retry with a narrower glob, a more specific query, or a lower max. Do not fall back to raw search commands.",
].join("\n");
